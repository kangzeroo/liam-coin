...
function shouldBehaveLikeERC20 (errorPrefix, initialSupply, initialHolder, recipient, anotherAccount) {
    ...
    describe('transfer', function () {
        shouldBehaveLikeERC20TokenTransfer(errorPrefix, initialHolder, recipient, initialSupply, function (from, to, value) {
            ...
            describe('when the recipient is not the zero address', function () {
                ...
                describe('when the spender has enough approved balance', function () {
                    ...
                    describe('when the token owner has enough balance', function () {
                        ...
                        it ('decreases the spenders allowance', async function () {
                            await this.token.transferFrom(tokenOwner, to, amount, { from: spender });
                    ...
                    describe('when the token owner does not have enough balance', function () {
                        ...
                        it ('reverts', async function () {
                            await expectRevert(this.token.transferFrom(tokenOwner, to, amount, { from: spender }),
                            `${errorPrefix}: Transfer amount exceeds balance`);
                        })
                    })

                    describe('when the recipient would overflow', function () {
                        const amount = initialSupply.addn(1);
                        it ('reverts', async function () {
                            await expectRevert(this.token.transferFrom(tokenOwner, to, amount, { from: spender }),
                            `${errorPrefix}: Transfer amount exceeds balance`);
                ...
                describe('when the spender does not have enough approved balance', function () {
                    beforeEach(async function () {
                        await this.token.approve(spender, initialSupply.subn(1), { from: tokenHolder });
                    })
                    describe('when the token owner has enough balance', function () {
                        const amount = initialSupply;
                        it ('reverts', async function () {
                ...
                describe('when the recipient is the zero address', function () {
                    ...
                    it ('reverts', async function () {
                        await expectRevert(this.token.transferFrom(tokenOwner, ZERO_ADDRESS, amount, { from: spender }),
                        `${errorPrefix}: Recipient is the zero address`);
                    })
                })
...
function shouldBehaveLikeERC20TokenTransfer(errorPrefix, from, to, balance, transfer) {
    describe('when the recipient is not the zero address', function () {
        describe('when the recipient does not have enough balance', function () {
    ...
    describe('when the recipient is the zero address', function () {
        const amount = balance;
        it ('reverts', async function () {
            await expectRevert(transfer.call(this, from, ZERO_ADDRESS, amount),
            `${errorPrefix}: Revert`);
        })
    })
}
...
function shouldBehaveLikeERC20Approve (errorPrefix, owner, spender, supply, approve) {
    describe('when the spender is not the zero address', function () {
        ...
        describe('when the sender does not have enough balance', function () {
            ...
            it ('emits an approval event regardless, because approvals can exceed balances', async function () {
                const { logs } = await approve.call(this, owner, spender, amount);
                expectEvent.inLogs(logs, 'Approval', {
                    owner, spender, value: amount
                })
...
module.exports = {
    ...
    shouldBehaveLikeERC20TokenTransfer,
